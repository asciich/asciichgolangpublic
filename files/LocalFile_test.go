	...
	for _, tt := range tests {
		t.Run(
			...
			func(t *testing.T) {
				...
				tempFile2.MustWriteString(tt.contentFile1, verbose)
	...
	for _, tt := range tests {
		t.Run(
			...
			func(t *testing.T) {
				...
				assert.False(pointersutils.MustPointersEqual(
...
func getRepoRootDir() (repoRoot Directory) {
	path := commandexecutor.Bash().MustRunOneLinerAndGetStdoutAsString("git show-toplevel .", false)
	...
	for _, tt := range tests {
		t.Run(
			...
			func(t *testing.T) {
				...
				assert.False(testFile.MustIsPgpEncrypted(verbose))
			},
		)
	}
}
