...
- SSL/TLS certificates:
    - The `httpoptions.RequestOptions` must contain an optional bool `CollectCertificates` to collect store the certificate used by the webserver.
        ...
        - This requires an `Example_GetRequestAndLogCertificateInfos_test.go` for documentation purposes.

## Testing

- Tests in the `httputils` package are meant to run the same logic against all implementations to ensure they behave in the same way.
    - Every function must be tested this way.
- `Example_*_test.go` packages are meant for documentation purposes and can use the convenience functions in `httputils` directly. If not asked explicitly they must not run against all implementations.
