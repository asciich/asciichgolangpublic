...
func TestGenerateSshKeyPair_HappyPath(t *testing.T) {
	...
	_, err := sshutils.GenerateSshKeyPair(ctx, &sshoptions.GenerateKeyOptions{
...
func TestGenerateSshKeyPair_KeysAreUnique(t *testing.T) {
	...
	_, err := sshutils.GenerateSshKeyPair(ctx, &sshoptions.GenerateKeyOptions{
		PrivateKeyPath: privPath1,
		PublicKeyPath:  pubPath1,
	})
	require.NoError(t, err, "first call failed")

	_, err = sshutils.GenerateSshKeyPair(ctx, &sshoptions.GenerateKeyOptions{
		PrivateKeyPath: privPath2,
		PublicKeyPath:  pubPath2,
	})
...
func TestReturnedSshKeyPair(t *testing.T) {
	ctx := contextutils.ContextVerbose()

	keyPair, err := sshutils.GenerateSshKeyPair(ctx, &sshoptions.GenerateKeyOptions{})
	require.NoError(t, err)
