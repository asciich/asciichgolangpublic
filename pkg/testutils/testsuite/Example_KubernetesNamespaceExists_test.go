...
// Test_Example_KubernetesNamespaceExists_SSH tests running namespace existence checks over SSH to a pod in a Kind cluster.
// It demonstrates:
// 1. Starting a Kind cluster
// 2. Creating a namespace for testing
// 3. Setting up an SSH server pod with key-based authentication
// 4. Using port forwarding to access the SSH server
// 5. Running kubernetes_namespace_exists tests over SSH
func Test_Example_KubernetesNamespaceExists_SSH(t *testing.T) {
	ctx := contextutils.ContextVerbose()
	...
	const namespaceName = "namespace-ssh-test"
	const podName = "ssh-server-namespace"
	...
	testSuite := &testsuite.TestSuite{
		Name:                  "SSH Namespace exists test",
		Description:           "Test SSH kubernetes_namespace_exists execution on Kubernetes pod",
		SSHHost:               "localhost",
		SSHUser:               "testuser",
		...
		TestCases: []*testcase.TestCase{
			{
				Name:        "Test namespace exists via SSH",
				TestType:    "kubernetes_namespace_exists",
				Namespace:   namespaceName,
				Cluster:     "kind-asciichgolangpublic",
				Description: "Check that an existing namespace is detected via SSH",
			},
			{
				Name:        "Test nonexistent namespace via SSH",
				TestType:    "kubernetes_namespace_exists",
				Namespace:   "namespace-does-not-exist-ssh",
				Cluster:     "kind-asciichgolangpublic",
				Description: "Check that a nonexistent namespace is detected via SSH",
			},
		},
	}
	...
	passed, err := result.GetNPassed(ctx)
	require.NoError(t, err)
	require.EqualValues(t, 1, passed, "One test should pass (existing namespace)")

	failed, err := result.GetNFailed(ctx)
	require.NoError(t, err)
	require.EqualValues(t, 1, failed, "One test should fail (nonexistent namespace)")
	...
	t.Logf("SSH Namespace test completed successfully on %s:%d!", setupResult.NamespaceName, setupResult.LocalPort)
}
