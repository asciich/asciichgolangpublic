...
These file contains the specification for the `testsuite` package and it's subpackages.
...
- For test requiring a jumphost emulation a SSH server is started inside the kind cluster:
    - The `SetupSSHServerInKind` setups up the SSH server pod and ensures the needed tools are installed.
