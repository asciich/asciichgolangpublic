...
func (t *TestCaseExecutorKubernetesCronJobExists) Run(ctx context.Context, commandExecutor commandexecutorinterfaces.CommandExecutor) (testutilsinterfaces.TestResult, error) {
	tStart := time.Now()
	...
	exists, err = ns.CronJobByNameExists(ctx, cronJobName)
	if err != nil {
		return nil, err
	}
