...
func Test_Example_IsStaticallyLinkedBinary(t *testing.T) {
	...
	isStaticallyLinked, err := filesutils.IsStaticallyLinkedBinary(ctx, "/bin/ls")
	...
	require.NoError(t, err)
	defer filesutils.Delete(ctx, tempDir, &filesoptions.DeleteOptions{})
...
func main() {}
`
	err = filesutils.WriteString(ctx, sourcePath, sourceCode)
	require.NoError(t, err)
	...
	isStaticallyLinked, err = filesutils.IsStaticallyLinkedBinary(ctx, binaryPath)
	require.NoError(t, err)
