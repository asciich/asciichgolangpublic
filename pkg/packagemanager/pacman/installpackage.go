...
func InstallPackages(ctx context.Context, commandExecutor commandexecutorinterfaces.CommandExecutor, packageNames []string, options *packagemanageroptions.InstallPackageOptions) error {
	...
	logging.LogInfoByCtxf(ctx, "Install pacman packages '%v' started.", packageNames)
	...
	if isInstalled {
		logging.LogInfoByCtxf(ctx, "Pacman packages '%s' is already installed.", packageNames)
	} else {
		logging.LogInfoByCtxf(ctx, "Pacman packages '%s' is not installed. Going to install it now.", packageNames)
		...
		logging.LogChangedByCtxf(ctx, "Installed pacman packages '%s'", packageNames)
	}
	...
	logging.LogInfoByCtxf(ctx, "Install pacman package '%s' finished.", packageNames)

	return nil
}

func (p *Pacman) InstallPackages(ctx context.Context, packageNames []string, options *packagemanageroptions.InstallPackageOptions) error {
