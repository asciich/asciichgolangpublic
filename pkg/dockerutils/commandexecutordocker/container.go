...
func (c *CommandExecutorDockerContainer) GetContainerStateStatus(ctx context.Context) (string, error) {
	...
	for i := range 10 {
		...
		if !output.IsExitSuccess() {
			...
			if strings.Contains(stderr, "error: no such object:") {
	...
	if status == "" {
		return "", tracederrors.TracedErrorf("Failed to evaluate .State.Status for docker container '%s'. status is empty string after evaluation.", containerName)
	}
