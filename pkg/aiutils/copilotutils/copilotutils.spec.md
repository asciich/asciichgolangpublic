...
- `RunCopilotCliContainer` can be used to spin a container to running copilot CLI as agent in an isolated way.
    - The argument of type `RunCopilotCliContainerOptions` provides at least:
        ...
        - `SSHAgent`. If set to true the `SSH_AUTH_SOCK` env var is passed into the container. Furthermore the socket is mounted insied the container as well, this way the ssh-agent can be used inside the container and is available for clone git repos.
    ...
    - Pass ``OPILOT_GITHUB_TOKEN` for authentication. If not set return an error and do not even start the container.
    - Use a generated container name: `copliot-cli-<8randomchars>`.
