...
// Already moved to nativefilesoo
func (l *LocalFile) OpenAsReadCloser(ctx context.Context) (io.ReadCloser, error) {
	path, err := l.GetPath()
	if err != nil {
		return nil, err
	}

	nativeFile, err := nativefilesoo.NewFileByPath(path)
	if err != nil {
		return nil, err
	}
...
func (l *LocalFile) GetBaseName() (baseName string, err error) {
	...
	if baseName == "" {
		return "", tracederrors.TracedErrorf("Base name is empty string after evaluation of path='%s'", path)
	}
