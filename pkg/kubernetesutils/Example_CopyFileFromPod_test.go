...
func ExampleCopyFileFromPod_nativeKubernetes() {
	...
	fmt.Printf("Successfully copied file from %s in pod %s to local %s. Content matches: %v\n", srcPathInPod, podName, destFile, string(content) == testContent)

	// Output: Successfully copied file from /tmp/example-source-file.txt in pod example-copy-from-pod to local /tmp/*.txt. Content matches: true
}
...
func ExampleCopyFileFromPod_commandExecutor() {
	...
	fmt.Printf("Successfully copied file from %s in pod %s to local %s using kubectl cp. Content matches: %v\n", srcPathInPod, podName, destFile, string(content) == testContent)

	// Output: Successfully copied file from /tmp/example-cmd-source.txt in pod example-copy-from-cmd-pod to local /tmp/*.txt using kubectl cp. Content matches: true
}
...
func ExampleCopyFileFromPod_roundTrip() {
	...
	fmt.Printf("Step 2: Copied file from pod to local at %s\n", retrievedFile)
	...
	// Step 2: Copied file from pod to local at /tmp/*.txt
