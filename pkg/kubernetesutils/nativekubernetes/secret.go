...
func CreateSecret(ctx context.Context, clientset *kubernetes.Clientset, namespaceName string, secretName string, data map[string][]byte) error {
	...
	if err != nil {
		if apierrors.IsAlreadyExists(err) {
			logging.LogInfoByCtxf(ctx, "Secret '%s' in namespace '%s' already exists.", secretName, namespaceName)
			return nil
		}
		return tracederrors.TracedErrorf("Failed to create secret '%s' in namespace '%s': %w", secretName, namespaceName, err)
	}
