...
func NewInstallPackagesCmd() *cobra.Command {
	const short = "Install packages using yay."
...
Usage if yay already installed:
    ` + os.Args[0] + `packagemanager yay install-packages package1 [package2...]
...
This command is idempotent: If yay is already installed it will be used without reinstallation.
	` + os.Args[0] + `packagemanager yay install-packages --install-yay --use-sudo package1 [package2...]`,
		Run: func(cmd *cobra.Command, args []string) {
			...
			if len(args) == 0 {
				logging.LogFatalf("Please specify packages to install.")
			}
			packages := args
			...
			mustutils.Must0(yay.InstallPackages(ctx, commandExecutor, packages, &packagemanageroptions.InstallPackageOptions{
				UpdateDatabaseFirst: true,
				UseSudo:             true,
			}))

			logging.LogGoodByCtxf(ctx, "Installed yay packages '%v' on '%s'.", packages, hostDescription)
	...
	cmd.Flags().Bool("use-sudo", false, "Use sudo to gain permissions to update when needed during the installation.")
