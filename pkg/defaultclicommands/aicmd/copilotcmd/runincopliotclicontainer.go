...
`,
		Run: func(cmd *cobra.Command, args []string) {
			...
			keepStoppedContainer, err := cmd.Flags().GetBool("keep-stopped-container")
			if err != nil {
				logging.LogGoErrorFatal(err)
			}
			...
			options.KeepStoppedContainer = keepStoppedContainer

			options.SetSSHAgent(sshAgent)

			output := mustutils.Must(copilotutils.RunCopilotCliContainer(options))

			logging.LogInfoByCtxf(ctx, "Copilot CLI output:\n%s", mustutils.Must(output.GetStdoutAsString()))
			logging.LogInfoByCtxf(ctx, "Stderr CLI output:\n%s", mustutils.Must(output.GetStderrAsString()))
	...
	cmd.Flags().Bool("keep-stopped-container", false, "Keep the container after it has stopped instead of removing it. Useful for debugging.")
