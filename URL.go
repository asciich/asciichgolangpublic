...
func (u *URL) GetAsString() (urlString string, err error) {
	if len(u.urlString) <= 0 {
		return "", TracedError("urlString not set")
	}
...
func (u *URL) GetPathAsString() (urlPath string, err error) {
	...
	if len(splitted) <= 0 {
		return "", TracedError("failed to split 'withoutSheme'")
	}
...
func (u *URL) GetPathBasename() (basename string, err error) {
	...
	if len(splitted) <= 0 {
		return "", TracedErrorf("failed to split '%v'", path)
	}
...
func (u *URL) GetSheme() (sheme string, err error) {
	...
	sheme = splitted[0]
	if sheme == "" {
		return "", TracedError("sheme is empty string after evaluation")
	}
...
func (u *URL) GetUrlString() (urlString string, err error) {
	if u.urlString == "" {
		return "", TracedErrorf("urlString not set")
	}
...
func (u *URL) SetByString(urlString string) (err error) {
	...
	if len(urlString) <= 0 {
		return TracedError("urlString is empty string")
	}
...
func (u *URL) SetUrlString(urlString string) (err error) {
	if urlString == "" {
		return TracedErrorf("urlString is empty string")
	}
